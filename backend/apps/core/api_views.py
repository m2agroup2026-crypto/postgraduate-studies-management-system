...
ROLE_IDENTITIES = {
    "DEAN": {
        "name": "الأستاذ الدكتور علاء عطية",
        "title": "عميد كلية الطب",
    },
    "VICE_DEAN": {
        "name": "الأستاذ الدكتور محمد عبد الباسط خلاف",
        "title": "وكيل الكلية لشؤون الدراسات العليا والبحوث",
    },
...
class DashboardView(APIView):
    def get(self, request):
        identity = ROLE_IDENTITIES.get(
            request.user.role,
            ROLE_IDENTITIES["PROGRAM_DIRECTOR"],
        )
        departments = list(
        ...
        return Response(
            {
                ...
                "alerts": [
                    {
                        "level": "warning",
                        "text": "ملفات تحتاج مراجعة قبل اجتماع اللجنة القادم",
                    },
                    {
                        "level": "info",
                        "text": "مناقشات مقررة خلال الثلاثين يومًا القادمة",
                    },
                ],
            }
        )
...
class DashboardConfigurationView(APIView):
    def get(self, request):
        if not can_manage_dashboard(request.user):
            return Response(
                {"error": "غير مصرح لك بإدارة إعدادات لوحة التحكم"},
                status=403,
            )
        return Response(
            {
    ...
    def patch(self, request):
        ...
        if not isinstance(navigation, list) or not isinstance(metrics, list):
            return Response(
                {"error": "صيغة إعدادات لوحة التحكم غير صحيحة"},
                status=400,
            )
        ...
        try:
            for payload in navigation:
                ...
                if not key:
                    return Response(
                        {"error": "كل عنصر قائمة يجب أن يحتوي على key"},
                        status=400,
                    )
                try:
                    ...
                    return Response(
                        {"error": f"عنصر القائمة غير معروف: {key}"},
                        status=400,
                    )
            ...
            for payload in metrics:
                ...
                if not key:
                    return Response(
                        {"error": "كل بطاقة مؤشر يجب أن تحتوي على key"},
                        status=400,
                    )
                try:
                    ...
                    return Response(
                        {"error": f"بطاقة المؤشر غير معروفة: {key}"},
                        status=400,
                    )
...
class AssistantView(APIView):
    def post(self, request):
        ...
        if "متأخر" in normalized or "pending" in normalized:
            answer = (
                "يوجد "
                f"{Thesis.objects.exclude(status='COMPLETED').count()} "
                "ملفًا قيد المتابعة."
            )
        elif "مناقش" in normalized or "defense" in normalized:
            answer = (
                "إجمالي لجان المناقشة المسجلة حاليًا "
                f"{DefenseCommittee.objects.count()}."
            )
        elif "طالب" in normalized or "student" in normalized:
            answer = f"إجمالي الطلاب المسجلين في النظام {Student.objects.count()}."
        else:
            answer = (
                "أستطيع مساعدتك في الطلاب والرسائل والمناقشات "
                "والملفات المتأخرة والتقارير."
            )
        return Response(
            {
                "answer": answer,
